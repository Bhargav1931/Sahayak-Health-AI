...
# it centrally here so no week-N notebook has to catch it.
...
def build_evaluation_dataset(n: int = 50, seed: int = 42) -> pd.DataFrame:
    """Return the locked 50-case evaluation dataset used across all four weeks.
    ...
    Every notebook, every week, every metric is measured against this
    identical 50-case set so numbers are directly comparable across weeks.
