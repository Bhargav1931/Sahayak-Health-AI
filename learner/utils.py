...
Contains four groups:
  ...
  4. Model factory       -- auto-selects Gemini or Ollama; shared across all weeks
  5. Safety constants    -- DISCLAIMER + safety_evaluator_agent (Week 1 preview)
"""
...
def safety_evaluator_agent(
    ...
    """Rule-based safety evaluator -- GIVEN for Week 1 preview; learners rebuild in Week 4.
...
#    Week 4). These are pure infrastructure: running a pipeline, parsing its
#    output, and scoring a results table. They contain NO triage answers --
#    run_policy_evaluation drives YOUR own Week-2 run_policy_triage().
# -----------------------------------------------------------------------------
...
async def run_triage_async(runner, session_service, patient_input, session_id=None, user_id="priya"):
    ...
    state (every agent's output_key). This is the call pattern you read in Week 3."""
...
def run_policy_evaluation(n=50, seed=42):
    ...
    GIVEN harness -- it drives YOUR Week-2 run_policy_triage() from
    ...
    try:
        ...
        raise RuntimeError(
            "run_policy_evaluation needs your Week-2 run_policy_triage() in "
            "sahayak_starter.py -- implement it before running the Week-4 baseline."
        ) from exc
